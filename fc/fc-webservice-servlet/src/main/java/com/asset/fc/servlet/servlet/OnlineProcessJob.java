 ...
 * @author nour.ihab
 */
public class OnlineProcessJob extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            job.setBody(body);
            ParserThread parse = new ParserThread(job, true);
            parse.run();

            if (parse.getEx() == null) {
                resp.getWriter().append("Accepted, the " + job.getJobName() + " has finished and its ID is: " + job.getJobId()).flush();
            } else {
                resp.getWriter().append("Failed to finsih: " + job.getJobName() + "with id= " + job.getJobId() + "and the error is: " + parse.getEx().getMessage());
